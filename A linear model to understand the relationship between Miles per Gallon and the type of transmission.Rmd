...
MPG is used as a measure of car performance. An automobile with 24 MPG is going to be less efficient than one with 30 MPG. The reason lies in the fact that a lower MPG indicates that a car uses more fuel per mile. 
...
The second model shows that the average MPG on our data set is 37.32155 . From the model, we can also infer that a loss of 5.35281 in MPG should be expected by a 1000 pounds increase in weight on a given car [2]. Finally, the model suggests that a decrease of 0.02362 in MPG can be expected with cars equipped with a manual transmission.
